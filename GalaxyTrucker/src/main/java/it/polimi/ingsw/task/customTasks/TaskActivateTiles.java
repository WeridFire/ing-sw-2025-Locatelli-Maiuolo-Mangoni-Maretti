...
public class TaskActivateTiles extends Task {
	...
	public TaskActivateTiles(String currentPlayer, int cooldown, PowerType powerType, BiConsumer<Player, Set<Coordinates>> onFinish) {
		super(currentPlayer, cooldown, TaskType.ACTIVATE_TILE);
	...
	protected void finish() {
		onFinish.accept(getPlayer(), activatedTiles);
	}
	...
	public void activateTiles(Player player, Set<Coordinates> coordinates) throws WrongPlayerTurnException, NotEnoughItemsException, TileNotAvailableException {
		...
		if (getAvailableBatteriesAmount() >= coordinates.size()) {
			getActivatedTiles().addAll(coordinates);
	 ...
	 */
	public int getAvailableBatteriesAmount() {
		return (getPlayer().getShipBoard().getVisitorCalculateCargoInfo()
